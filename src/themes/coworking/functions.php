...
function uni_coworking_theme_filter_demo_header_list() {
	echo '<li id="theme-version"><span>Coworking Co. WP Theme 1.1.7</span></li>';
}
...
if ( class_exists('Coworking_Plugin') ) {
...
function uni_coworking_theme_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			'name'               => 'Coworking Plugin',
			'slug'               => 'coworking-plugin',
			'source'             => 'http://highseastudio.com/plugins/coworking-plugin.zip',
			'required'           => false,
			'version'            => '1.0.0',
        ...
        array(
			'name'               => 'Envato Market',
			'slug'               => 'envato-market',
			'source'             => 'http://highseastudio.com/plugins/envato-market.zip',
			'required'           => false,
			'version'            => '1.0.0-RC2',
			'force_activation'   => false,
			'force_deactivation' => false,
		...
		array(
			...
			'source'             => 'http://highseastudio.com/plugins/coworking/uni-cpt-and-tax.zip',
			'required'           => true,
			'version'            => '1.3.2',
			'force_activation'   => false,
			'force_deactivation' => false,
		...
		array(
			...
			'source'             => 'http://highseastudio.com/plugins/uni-sortable-users.zip',
			'required'           => false,
			'version'            => '1.0.0',
			'force_activation'   => false,
			'force_deactivation' => false,
		...
		array(
			...
			'source'             => 'http://highseastudio.com/plugins/uni-calendarius.zip',
			'required'           => true,
			'version'            => '1.2.4',
			'force_activation'   => false,
			'force_deactivation' => false,
		...
		array(
			...
			'slug'               => 'uni-profilini',
			'source'             => 'http://highseastudio.com/plugins/uni-profilini.zip',
			'required'           => false,
			'version'            => '2.0.0-beta',
			'force_activation'   => false,
			'force_deactivation' => false,
		...
		array(
			'name'     => 'WooCommerce Product Options and Price Calculation Formulas - Uni CPO',
	...
	$config = array(
		...
		'strings'      => array(
			...
			'installing'                      => esc_html__( 'Installing Plugin: %s', 'coworking' ),
			// %s = plugin name.
			...
			'plugin_already_active'           => esc_html__( 'No action taken. Plugin %1$s was already active.', 'coworking' ),
			// %1$s = plugin name(s).
			'plugin_needs_higher_version'     => esc_html__( 'Plugin not activated. A higher version of %s is needed for this theme. Please update the plugin.', 'coworking' ),
			// %1$s = plugin name(s).
			'complete'                        => esc_html__( 'All plugins installed and activated successfully. %1$s', 'coworking' ),
			// %s = dashboard link.
...
function uni_coworking_theme_scripts() {
	...
	// parsley
	wp_register_script( 'parsleyjs', get_template_directory_uri() . '/js/parsley.min.js', array( 'jquery' ), '2.8.0' );
	wp_enqueue_script( 'parsleyjs' );
	// parsley localization
	wp_register_script( 'parsley-localization', get_template_directory_uri() . '/js/parsley/i18n/en.js', array( 'parsleyjs' ), '2.8.0' );
	wp_enqueue_script( 'parsley-localization' );
	// theme's scripts
	wp_enqueue_script(
		'uni-coworking-theme-script',
	...
	wp_localize_script( 'parsleyjs', 'uni_parsley_loc', $aParsleyStrings );
...
function uni_coworking_theme_enqueue_google_maps_script() {
	...
	} elseif ( ot_get_option( 'uni_gmaps_elsewhere_enable' ) == 'on' ) {
		if ( ! empty( $sApiKey ) ) {
			wp_enqueue_script( 'google-maps', '//maps.googleapis.com/maps/api/js?key=' . esc_attr( $sApiKey ) );
		} else {
			wp_enqueue_script( 'google-maps', '//maps.googleapis.com/maps/api/js', array(), '' );
...
function uni_coworking_theme_styles() {
	...
	if ( ! ot_get_option( 'uni_google_fonts' ) || empty( $ot_set_google_fonts ) ) {
		wp_enqueue_style( 'uni-coworking-theme-fonts', uni_coworking_theme_fonts_url(), array(), '1.1.7' );
	}
	...
	wp_enqueue_style( 'uni-coworking-theme-styles', get_template_directory_uri() . '/style.css', array(
		'bxslider',
		'remodal',
	...
	wp_enqueue_style( 'uni-coworking-theme-adaptive', get_template_directory_uri() . '/css/adaptive.css', array( 'uni-coworking-theme-styles' ), '1.1.7', 'screen' );
}
...
function uni_coworking_theme_header_div_output() {
	...
	$aLogoTransparentPadding = ( ! isset( $aLogoTransparentPadding['top'] ) || ! isset( $aLogoTransparentPadding['right'] ) || ! isset( $aLogoTransparentPadding['bottom'] ) || ! isset( $aLogoTransparentPadding['left'] ) || ! isset( $aLogoTransparentPadding['unit'] ) ) ? array(
		'top'    => 30,
		'right'  => 38,
		'bottom' => 26,
		'left'   => 39,
		'unit'   => 'px'
	) : ot_get_option( 'uni_logo_padding' );
	$aLogoStickyPadding      = ot_get_option( 'uni_logo_padding_sticky' );
	$aLogoStickyPadding      = ( ! isset( $aLogoStickyPadding['top'] ) || ! isset( $aLogoStickyPadding['right'] ) || ! isset( $aLogoStickyPadding['bottom'] ) || ! isset( $aLogoStickyPadding['left'] ) || ! isset( $aLogoStickyPadding['unit'] ) ) ? array(
		'top'    => 23,
		'right'  => 38,
		'bottom' => 21,
		'left'   => 39,
		'unit'   => 'px'
	) : ot_get_option( 'uni_logo_padding_sticky' );
    ...
    <header id="header" data-header-sticky-height="<?php echo esc_attr( $iLineHeightSticky + 1 ) ?>"
            style="height:<?php echo esc_attr( $iLineHeightTransparent + 1 ) ?>px;">
        <style scoped>
            .headerWrap.isSticky {
                line-height: <?php echo esc_attr($iLineHeightSticky) ?>px !important;
            }

            .logo {
                padding: <?php echo intval($aLogoTransparentPadding['top']).esc_attr($aLogoTransparentPadding['unit']); ?> <?php echo intval($aLogoTransparentPadding['right']).esc_attr($aLogoTransparentPadding['unit']); ?> <?php echo intval($aLogoTransparentPadding['bottom']).esc_attr($aLogoTransparentPadding['unit']); ?> <?php echo intval($aLogoTransparentPadding['left']).esc_attr($aLogoTransparentPadding['unit']); ?>;
            }

            .isSticky .logo {
                padding: <?php echo intval($aLogoStickyPadding['top']).esc_attr($aLogoStickyPadding['unit']); ?> <?php echo intval($aLogoStickyPadding['right']).esc_attr($aLogoStickyPadding['unit']); ?> <?php echo intval($aLogoStickyPadding['bottom']).esc_attr($aLogoStickyPadding['unit']); ?> <?php echo intval($aLogoStickyPadding['left']).esc_attr($aLogoStickyPadding['unit']); ?>;
            }

            <?php if ( ot_get_option('uni_header_book_tour_link_enable') != 'off' ) { ?>
            .bookATour {
                line-height: <?php echo esc_attr($iLineHeightTransparent) ?>px;
                padding: 0 41px 0 39px;
            }

            .isSticky .bookATour {
                line-height: <?php echo esc_attr($iLineHeightSticky) ?>px;
            }
        ...
        <div class="headerWrap uni-clear" style="line-height:<?php echo esc_attr( $iLineHeightTransparent ) ?>px;">
			...
			<?php } elseif ( ot_get_option( 'uni_logo_header_type' ) === 'text' ) {
				$logo_text = ot_get_option( 'uni_logo_header_text' );
				?>
			...
			<?php wp_nav_menu( array(
				...
				'menu_class'      => 'uni-clear',
				'theme_location'  => 'primary',
				'depth'           => 3,
				'fallback_cb'     => 'uni_coworking_theme_nav_fallback'
			) ); ?>

			<?php if ( class_exists('Coworking_Plugin') && ot_get_option( 'uni_header_book_tour_link_enable' ) != 'off' ) { ?>
				<?php if ( ot_get_option( 'uni_header_book_tour_link_custom_enable' ) != 'on' ) { ?>
                    <a data-remodal-target="bookingForm" href=""
                       class="bookATour"><?php if ( ot_get_option( 'uni_header_book_tour_link_text' ) ) {
							$sBookTourLink = ot_get_option( 'uni_header_book_tour_link_text' );
							echo esc_html( $sBookTourLink );
						} else {
							esc_html_e( 'book a tour', 'coworking' );
						} ?></a>
				<?php } else { ?>
                    <a href="<?php echo esc_url( ot_get_option( 'uni_header_book_tour_link_custom_uri' ) ); ?>"
                       class="bookATour"
                       target="_blank"><?php if ( ot_get_option( 'uni_header_book_tour_link_text' ) ) {
							$sBookTourLink = ot_get_option( 'uni_header_book_tour_link_text' );
							echo esc_html( $sBookTourLink );
						} else {
							esc_html_e( 'book a tour', 'coworking' );
...
//
add_action( 'uni_coworking_theme_header_wrapper_before', 'uni_coworking_theme_loader', 9 );
...
function uni_coworking_theme_get_custom_logo_a( $blog_id = 0 ) {
	...
	if ( $sLogoAId ) {
		...
		$html .= '<img class="logo-black uni-custom-logo-a" src="' . get_template_directory_uri() . '/images/logo.png" alt="' . esc_attr( get_bloginfo( 'description' ) ) . '">';
	}
...
function uni_coworking_theme_get_custom_logo_b( $blog_id = 0 ) {
	...
	if ( $sLogoBId ) {
		...
		$html .= '<img class="logo-white uni-custom-logo-b" src="' . get_template_directory_uri() . '/images/logo-white.png" alt="' . esc_attr( get_bloginfo( 'description' ) ) . '">';
	}
...
function uni_coworking_theme_get_custom_logo_footer( $blog_id = 0 ) {
	...
	if ( $sLogoBId ) {
		...
		$html .= '<img class="uni-custom-logo-footer" src="' . get_template_directory_uri() . '/images/footerlogo.png" alt="' . esc_attr( get_bloginfo( 'description' ) ) . '">';
	}
...
function uni_coworking_theme_mobile_menu_output() {
    ...
    <div class="mobileMenu">
		...
		<?php wp_nav_menu( array(
			...
			'theme_location'  => 'primary',
			'depth'           => 3,
			'fallback_cb'     => 'uni_coworking_theme_nav_mobile_fallback'
		) ); ?>
...
function uni_coworking_theme_booking_form_output() {
	if ( ot_get_option( 'uni_header_book_tour_link_enable' ) !== 'off' ||
	     ( ot_get_option( 'uni_header_book_tour_link_enable' ) === 'off' && is_page_template( 'templ-home.php' ) )
	) {
		?>
        <div class="bookingForm" data-remodal-id="bookingForm">
            <span data-remodal-action="close"
                  class="thmRemodalClose"><?php esc_html_e( 'Close', 'coworking' ); ?></span>
            <div class="remodalFormWrap uni-clear">
                <h3><?php if ( ot_get_option( 'uni_header_book_tour_link_text' ) ) {
						$sBookTourLink = ot_get_option( 'uni_header_book_tour_link_text' );
						echo esc_html( $sBookTourLink );
					} else {
						esc_html_e( 'Book a Tour', 'coworking' );
                ...
                <form action="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>" method="post"
                    ...
                    <p><?php esc_html_e( 'Would you like to see our space before joining? Come and visit our coworking space. Please fill out the form and our manager will get back asap.', 'coworking' ); ?></p>
                    <div class="inputWrap">
                        <input type="text" name="uni_contact_firstname" value=""
                               placeholder="<?php esc_html_e( 'First Name', 'coworking' ); ?>"
                               data-parsley-required="true" data-parsley-trigger="change focusout submit">
                    </div>
                    <div class="inputWrap">
                        <input type="text" name="uni_contact_lastname" value=""
                               placeholder="<?php esc_html_e( 'Last Name', 'coworking' ); ?>"
                               data-parsley-required="true" data-parsley-trigger="change focusout submit">
                    </div>
                    <div class="inputWrap">
                        <input type="email" name="uni_contact_email" value=""
                               placeholder="<?php esc_html_e( 'Email', 'coworking' ); ?>" data-parsley-required="true"
                               data-parsley-trigger="change focusout submit" data-parsley-type="email">
                    </div>
                    <div class="inputWrap">
                        <input type="tel" name="uni_contact_phone" value=""
                               placeholder="<?php esc_html_e( 'Phone', 'coworking' ); ?>" data-parsley-required="true"
                    ...
                    <input class="thmSubmitBtn uni_input_submit" type="button"
                           value="<?php esc_html_e( 'request to book', 'coworking' ); ?>">
                </form>
            </div>
        </div>
		<?php
        ...
        <div class="joinForm" data-remodal-id="joinForm">
            ...
            <div class="remodalFormWrap uni-clear">
                ...
                <form action="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>" method="post"
                    ...
                    <p><?php esc_html_e( 'Join our great coworking space and community. Please fill out the form and our manager will get back asap.', 'coworking' ); ?></p>
                    <div class="inputWrap">
                        <input type="text" name="uni_contact_firstname" value=""
                               placeholder="<?php esc_html_e( 'First Name', 'coworking' ); ?>"
                               data-parsley-required="true" data-parsley-trigger="change focusout submit">
        ...
        <div class="priceForm" data-remodal-id="priceForm">
            ...
            <div class="remodalFormWrap uni-clear">
                ...
                <form action="<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>" method="post"
                    ...
                    <p><?php esc_html_e( 'Please fill out the form and our manager will get back asap.', 'coworking' ); ?></p>
                    <div class="inputWrap">
                        <input type="text" name="uni_contact_firstname" value=""
                               placeholder="<?php esc_html_e( 'First Name', 'coworking' ) ?>"
                               data-parsley-required="true" data-parsley-trigger="change focusout submit">
                    ...
                    <div class="inputWrap">
                        <input type="text" name="uni_contact_email" value=""
                               placeholder="<?php esc_html_e( 'Your Email', 'coworking' ) ?>"
                               data-parsley-required="true" data-parsley-trigger="change focusout submit"
                               data-parsley-type="email">
                    </div>
                    <div class="inputWrap">
                        <input type="text" name="uni_contact_phone" value=""
                               placeholder="<?php esc_html_e( 'Phone Number', 'coworking' ) ?>"
                               data-parsley-required="true" data-parsley-trigger="change focusout submit"
                    ...
                    <div class="textareaWrap">
                        <textarea name="uni_contact_msg" cols="30" rows="10"
                                  placeholder="<?php esc_html_e( 'Message', 'coworking' ) ?>"
					...
					<?php } ?>
                    <input class="thmSubmitBtn uni_input_submit" type="button"
                           value="<?php esc_html_e( 'Send', 'coworking' ) ?>">
                </form>
			...
			if ( ! empty( $sFontNameOne ) && $sFontNameOne != 'Lato' ) {
                ...
                <style type="text/css">
                    ...
                    select, textarea, input, body, .tickera-checkout {
                        font-family: '<?php echo esc_attr( $sFontNameOne ); ?>';
                    }
                </style>
				<?php
			}
			...
			if ( ! empty( $sFontNameTwo ) && $sFontNameTwo != 'Montserrat' ) {
                ...
                <style type="text/css">
                    ...
                    .mainMenu > ul > li > a, .mainMenu > ul > li > a:visited, .bookATour, .logo-text {
                        font-family: '<?php echo esc_attr( $sFontNameTwo ); ?>';
                    }
                </style>
				<?php
			}
	...
	if ( ( isset( $sColourOne ) && isset( $sColourTwo ) && ! empty( $sColourOne ) && ! empty( $sColourTwo ) )
        ...
        <style type="text/css">
            ...
            .coupon input[name="apply_coupon"]:hover {
                background-color: <?php echo esc_attr( $sColourOne ); ?> !important;
            }
            ...
            .coupon input[name="apply_coupon"] {
                border-color: <?php echo esc_attr( $sColourOne ); ?> !important;
            }
            ...
            .woocommerce-breadcrumb a:hover, .coupon input[name="apply_coupon"] {
                color: <?php echo esc_attr( $sColourOne ); ?> !important;
            }
            ...
            .bookATourLink, .bookATourLink:visited, .bookATour:before {
                background: <?php echo esc_attr( $sColourOne ); ?>;
            }
            ...
            .shopLink, .shopLink:visited, .blogLink, .blogLink:visited, .aboutLink, .aboutLink:visited {
                border-color: <?php echo esc_attr( $sColourOne ); ?>;
            }
            ...
            .page-template-templ-events-tickera .eventItem:hover .attendBtn svg path {
                fill: <?php echo esc_attr( $sColourOne ); ?>;
            }
            ...
            .postItemCategory:hover, .postItem:hover h3 a {
                color: <?php echo esc_attr( $sColourOne ); ?>;
            }
            ...
            .bookATourLink:hover {
                background-color: <?php echo esc_attr( $sColourTwo ); ?>;
            }

            .btnCheckout:hover {
                border-color: <?php echo esc_attr( $sColourTwo ); ?>;
            }
        </style>
		<?php
	}
...
function uni_coworking_theme_admin_script( $hook ) {
	...
	wp_enqueue_script( 'uni-coworking-theme-admin', get_template_directory_uri() . '/js/uni-admin.js', array( 'jquery' ), '1.1.7' );

    if ( ! empty( $_GET['post'] ) ) {
        wp_localize_script( 'uni-coworking-theme-admin', 'data_page', array(
...
function uni_coworking_theme_activation_func() {
	add_role( 'coworking_staff', esc_html__( 'Coworking Staff', 'coworking' ), array( 'read' => true ) );
			...
			while ( $oRelatedQuery->have_posts() ) {
                ...
                <div id="post-<?php the_ID(); ?>" <?php post_class( 'postItem' ) ?>>
					...
					<?php if ( has_excerpt( $post->ID ) ) {
						...
						uni_coworking_theme_excerpt( 15, '', true );
					} ?>
                </div>
			<?php }
			echo '</div>';
			...
			while ( $oRelatedQuery->have_posts() ) {
                ...
                <div id="post-<?php the_ID(); ?>" <?php post_class( 'relatedPostItem' ) ?>>
                    <a class="postItemImg" href="<?php the_permalink() ?>">
						...
						<?php } else { ?>
                            <img src="http://placehold.it/260x173" width="260" height="173"
                                 alt="<?php the_title_attribute() ?>"/>
						<?php } ?>
                    </a>
                    <div class="postItemMeta">
...
function uni_coworking_theme_join_event_form() {
	...
	if ( $sCustomerFName && $sCustomerLName && $sCustomerEmail && $sCustomerPhone && $sCustomerMsg ) {
		...
		$sAdminEmail    = ( is_email( $sUniEmail ) ) ? sanitize_email( $sUniEmail ) : sanitize_email( get_bloginfo( 'admin_email' ) );
		...
		} elseif ( ! empty( $aCustomData['uni_event_date_start'][0] ) && ! empty( $aCustomData['uni_event_date_end'][0] ) &&
		           $aCustomData['uni_event_date_start'][0] != $aCustomData['uni_event_date_end'][0] ) {
			$iDateStartTimestamp = strtotime( $aCustomData['uni_event_date_start'][0] );
		...
		} elseif ( ! empty( $aCustomData['uni_event_time_start'][0] ) && ! empty( $aCustomData['uni_event_time_end'][0] ) &&
		           $aCustomData['uni_event_time_start'][0] != $aCustomData['uni_event_time_end'][0] ) {
			$iTimeStartTimestamp = strtotime( $aCustomData['uni_event_time_start'][0] );
		...
		$sSubjectText       = html_entity_decode( sprintf( esc_html__( 'Successful registration for %s', 'coworking' ), $sEventTitle ), ENT_COMPAT, 'UTF-8' );
		...
		$aMailVars          = array(
			...
			'$sEventDate'     => $sEventDate,
			'$sEventTime'     => $sEventTime,
			'$sEventLocation' => $sEventLocation,
			'$sEventPrice'    => $sEventPrice,
			'$sPhone'         => $sPhone,
			'$sEmail'         => $sEmail
		);
		...
		$sSubjectText       = html_entity_decode( sprintf( esc_html__( 'A new registration for %s', 'coworking' ), $sEventTitle ), ENT_COMPAT, 'UTF-8' );
...
function uni_coworking_theme_price_form() {
	...
	if ( $sCustomerFName && $sCustomerLName && $sCustomerEmail && $sCustomerPhone && $sCustomerMsg ) {
		...
		$post_type   = get_post_type( $iPriceId );
		if ( 'uni_price' === $post_type ) {
			$sPriceCurrency = ( isset( $aCustomData['uni_price_currency'][0] ) && ! empty( $aCustomData['uni_price_currency'][0] ) ) ? esc_html( $aCustomData['uni_price_currency'][0] ) : esc_html__( '- not specified -', 'coworking' );
			$sPriceVal      = ( isset( $aCustomData['uni_price_val'][0] ) && ! empty( $aCustomData['uni_price_val'][0] ) ) ? esc_html( $aCustomData['uni_price_val'][0] ) : esc_html__( '- not specified -', 'coworking' );
			$sPricePeriod   = ( isset( $aCustomData['uni_price_period'][0] ) && ! empty( $aCustomData['uni_price_period'][0] ) ) ? esc_html( $aCustomData['uni_price_period'][0] ) : esc_html__( '- not specified -', 'coworking' );
		} elseif ( 'uni_price_plan' === $post_type ) {
			$sPriceCurrency = ( isset( $aCustomData['uni_plan_currency'][0] ) && ! empty( $aCustomData['uni_plan_currency'][0] ) ) ? esc_html( $aCustomData['uni_plan_currency'][0] ) : esc_html__( '- not specified -', 'coworking' );
			$sPriceVal      = ( isset( $aCustomData['uni_plan_val'][0] ) && ! empty( $aCustomData['uni_plan_val'][0] ) ) ? esc_html( $aCustomData['uni_plan_val'][0] ) : esc_html__( '- not specified -', 'coworking' );
			$sPricePeriod   = ( isset( $aCustomData['uni_plan_period'][0] ) && ! empty( $aCustomData['uni_plan_period'][0] ) ) ? esc_html( $aCustomData['uni_plan_period'][0] ) : esc_html__( '- not specified -', 'coworking' );
		}
		$sPhone = ( ot_get_option( 'uni_home_contact_phone' ) ) ? esc_html( ot_get_option( 'uni_home_contact_phone' ) ) : '+88 (0) 101 0000 000';
		$sEmail = ( ot_get_option( 'uni_contact_email' ) ) ? sanitize_email( $sUniEmail ) : sanitize_email( get_bloginfo( 'admin_email' ) );
		...
		$sSubjectText       = html_entity_decode( sprintf( esc_html__( 'Successful request for %s', 'coworking' ), $sPriceTitle ), ENT_COMPAT, 'UTF-8' );
		$sEmailTemplateName = apply_filters( 'uni_coworking_theme_price_email_filter', '/email/price-guest.php', 'guest' );
		$aMailVars          = array(
			'$sPriceTitle'    => '"' . $sPriceTitle . '"',
			'$sPriceCurrency' => $sPriceCurrency,
			'$sPriceVal'      => $sPriceVal,
			'$sPricePeriod'   => $sPricePeriod,
			'$sPhone'         => $sPhone,
			'$sEmail'         => $sEmail
		);
		...
		$sSubjectText       = html_entity_decode( sprintf( esc_html__( 'A new request for %s', 'coworking' ), $sPriceTitle ), ENT_COMPAT, 'UTF-8' );
		$sClientName        = $sCustomerFName . ' ' . $sCustomerLName;
		$sClientTel         = $sCustomerPhone;
		$sClientEmail       = $sCustomerEmail;
		$sClientMsg         = $sCustomerMsg;
		...
		$aResult['status']  = 'success';
		$aResult['message'] = esc_html__( 'Thanks! Your request was successfully sent!', 'coworking' );
...
function uni_coworking_theme_book_tour_form() {
	...
	if ( $sCustomerFName && $sCustomerLName && $sCustomerEmail && $sCustomerPhone ) {
		...
		$sSubjectText       = html_entity_decode( esc_html__( 'Successful request for a tour', 'coworking' ) );
		$sEmailTemplateName = apply_filters( 'uni_coworking_theme_book_email_filter', '/email/book-guest.php', 'guest' );
		$aMailVars          = array( '$sPhone' => $sPhone, '$sEmail' => $sEmail );
		...
		$sSubjectText       = html_entity_decode( esc_html__( 'A new request for a tour', 'coworking' ) );
		$sClientName        = $sCustomerFName . ' ' . $sCustomerLName;
		$sClientTel         = $sCustomerPhone;
		$sClientEmail       = $sCustomerEmail;
...
function uni_coworking_theme_contact_form() {
	...
	if ( $sCustomerFName && $sCustomerLName && $sCustomerEmail && $sCustomerPhone && $sCustomerMsg ) {
		...
		if ( ! empty( $iPageId ) ) {
			...
			if ( ! empty( $aPostCustom['uni_contact_email_subject'][0] ) ) {
				$sSubjectText = sanitize_email( $aPostCustom['uni_contact_email_subject'][0] );
			} else {
				$sSubjectText = html_entity_decode( esc_html__( 'A new submission of the contact form', 'coworking' ) );
			}
		} else {
		...
		$sMessage =
			"<h3>" . sprintf( esc_html__( 'You have a new message sent from "%s"!', 'coworking' ), $sBlogName ) . "</h3>
                    ...
                    <br>
                    " . sprintf( esc_html__( 'Phone: %s', 'coworking' ), $sCustomerPhone ) . "
                    <br>
...
function uni_coworking_theme_mailchimp_subscribe_user() {
	...
	if ( ! empty( $sApiKey ) && ! empty( $sListId ) ) {
		...
		$aListOfMCVars = apply_filters( 'uni_coworking_theme_mailchimp_variables_filter', array(
			...
			<?php if ( sizeof( $woocommerce->cart->get_cart() ) > 0 ) :
				foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
					...
					if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
						...
						if ( ! empty( $cart_item['_uni_cpo_cart_item_id'] ) ) {
							$cpo_data_attr = ' data-cart_item_id="' . esc_attr( $cart_item['_uni_cpo_cart_item_id'] ) . '"';
						}
						?>
                        <div class="miniCartItem"
                             data-product_id="<?php echo esc_attr( $product_id ) ?>"<?php if ( ! empty( $cart_item['_uni_cpo_cart_item_id'] ) ) {
							echo wp_kses_post( $cpo_data_attr );
						} ?>>
                            <a
                                    ref="<?php echo esc_url( $product_permalink ); ?>"
                                    class="miniCartItemImg">
							...
							<?php
							if ( WC()->cart->get_item_data( $cart_item ) ) {
                ...
                <a href="<?php echo esc_url( wc_get_cart_url() ); ?>"
                   class="btnViewCart"><?php esc_html_e( 'view cart', 'coworking' ); ?></a>
                <a href="<?php echo esc_url( wc_get_checkout_url() ); ?>"
                   class="btnCheckout"><?php esc_html_e( 'checkout', 'coworking' ); ?></a>
    ...
    <script type="text/template" id="js-uni-calendar-info-event-tmpl">
        <div class="uni-ec-universal-container" style="border-color:<%= calEvent.borderColor %>;">
            ...
            <div class="uni-ec-form-content">
                ...
                <div class="uni-ec-form-section uni-ec-form-section-modal">
                    ...
                    <div class="uni-ec-info-section uni-ec-info-section-duration">
                            <span class="uni-ec-info-section-duration-allday">
	                            ...
	                            <?php esc_html_e( 'All day', 'coworking' ) ?>
                            </span>
                    </div>
                    <% } else { %>
                ...
                <span class="js-uni-event-modal-close-btn uni-event-modal-close-btn"><?php esc_html_e( 'Close', 'coworking' ) ?></span>
                ...
                <div class="uni-ec-form-section uni-ec-form-section-modal">
                    ...
                    <div class="uni-ec-info-section uni-ec-info-section-availability">
                            <span class="uni-ec-info-section-availability-active">
	                            ...
	                            <?php esc_html_e( 'Active', 'coworking' ) ?>
                            </span>
                    </div>
                    <% } else { %>
                    <div class="uni-ec-info-section uni-ec-info-section-availability">
                            <span class="uni-ec-info-section-availability-cancelled">
	                            ...
	                            <?php esc_html_e( 'Cancelled', 'coworking' ) ?>
                            </span>
                    </div>
                    <% } %>
            ...
            <div class="uni-ec-form-content">
                ...
                <div class="uni-ec-form-section uni-ec-form-section-modal">
                    ...
                    <div class="uni-ec-info-section uni-ec-info-section-description uni-ec-form-section-nice-scroll">
                        ...
                        <%= calEvent.meta.event_desc %>
                        <% } else { %>
						<?php esc_html_e( 'No description.', 'coworking' ) ?>
                        <% } %>
                    </div>
            ...
            <div class="uni-ec-form-content">
                ...
                <div class="uni-ec-form-section uni-ec-form-section-with-btn centered uni-ec-form-section-modal">
                    <a href="<%= calEvent.meta.event_page_uri %>" class="btn btn-inform"
                       target="_blank"><?php esc_html_e( 'The event page', 'coworking' ) ?></a>
                    <button class="js-uni-event-modal-close-btn btn"
                            type="button"><?php esc_html_e( 'Close', 'coworking' ) ?></button>
                </div>
