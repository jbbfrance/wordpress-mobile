        ...
        foreach( (array) $this->get_pages( $option ) as $page ) {
          ...
          if ( ! isset( $page['hidden_page'] ) && $screen->id == $this->page_hook[$page['id']] ) {
            ...
            if ( $show_buttons ) {
              ...
              echo isset( $show_buttons ) && $show_buttons ? '</form>' : '</div>';

              /* reset button */
              if ( $show_buttons ) {
                global $wp;
                echo '<form method="post" action="' . str_replace( '&settings-updated=true', '', $wp->request ) . '">';
                  ...
                  echo '<button type="submit" class="option-tree-ui-button button button-secondary left reset-settings" title="' . esc_html__( 'Reset Options', 'coworking' ) . '">' . esc_html__( 'Reset Options', 'coworking' ) . '</button>';
            ...
            if ( isset( $setting['type'] ) && isset( $input[$setting['id']] ) ) {
              ...
              if ( is_array( $input[$setting['id']] ) && in_array( $setting['type'], array( 'list-item', 'slider' ) ) ) {
                ...
                $required_setting = array(
                  array(
                    'id'        => 'title',
                    'label'     => esc_html__( 'Title', 'coworking' ),
                    'desc'      => '',
                    'std'       => '',
                    'type'      => 'text',
                    'rows'      => '',
