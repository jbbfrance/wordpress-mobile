	...
	<div class="comments-wrapper">
		...
		$custom_comment_form = array( 'fields' => apply_filters( 'comment_form_default_fields', array(
			...
			'email'  => '<div id="email-wrapper">' .
					'<input type="text" name="email" id="email" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" placeholder="' . __( 'Email' , 'bonfire' ) . '" size="22" tabindex="3" />' .
					'</div>',
			'url'    => '<div id="url-wrapper">' .
					'<input name="url" id="url" type="text" value="' . esc_url(  $commenter['comment_author_url'] ) . '" placeholder="' . __( 'Website (optional)' , 'bonfire' ) . '" size="22" tabindex="4" />' .
					'</div></div>') ),
			'comment_field' => '<div id="comment-wrapper">' .
					'<textarea name="comment" id="comment" cols="45" rows="10" tabindex="1" placeholder="' . __( 'Type your comment here to join or kick off the discussion...' , 'bonfire' ) . '"></textarea>' .
			...
			'label_submit' => __( 'SUBMIT' , 'bonfire' ),
		);
