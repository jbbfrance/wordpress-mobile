...
class WPForms_Form_Abandonment {
	...
	public function entries_table_views( $views, $form_data, $counts ) {
		...
		if ( $this->has_form_abandonment( $form_data ) ) {
			...
			$views['abandoned'] = sprintf(
				...
				$current === 'abandoned' ? ' class="current"' : '', esc_html__( 'Abandoned', 'wpforms-form-abandonment' ) . $abandoned
			);
		}
		...
		foreach ( $forms as $form_id => $entry ) {
			...
			if ( $avoid_dupes ) {
				...
				$exists = $wpdb->get_row( // phpcs:ignore
					$wpdb->prepare(
						"SELECT entry_id FROM {$wpdb->prefix}wpforms_entries WHERE `form_id` = %d AND `user_uuid` = %s AND `status` = 'abandoned' AND `date` >= DATE_SUB(NOW(),INTERVAL %d HOUR) LIMIT 1;",
						absint( $form_id ),
	...
	public function process_email( $process, $fields, $form_data, $notification_id, $context ) {
		...
		if ( ! $this->has_form_abandonment( $form_data ) && 'abandoned' === $context ) {
			...
			// notification is enable for form abandonment.
			if ( ! empty( $form_data['settings']['notifications'][ $notification_id ]['form_abandonment'] ) ) {
				return false;
			}
		}
