...
class Ajax {
	...
	public function init() {
		...
		\add_action( 'wpforms_builder_save_form_response_data', array( $this, 'change_form_slug' ), 10, 3 );
	}
	...
	public function change_form_slug( $response, $form_id, $data ) {
		...
		if ( empty( $data['settings']['conversational_forms_enable'] ) ) {
			return $response;
		}
		...
		if ( empty( $form_slug ) ) {
			...
			$response['conversational_forms'] = array(
